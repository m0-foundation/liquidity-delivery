...
impl EvmEventListener {
    ...
    async fn process_log<P: Provider>(
        ...
        if event_signature == OrderOpened::SIGNATURE_HASH {
            let event = Self::handle_order_open(
                chain_id,
                &log_data,
                contract_address,
                provider,
            ...
            return Ok(Some(Self::handle_fill(
                &log_data,
                transaction_hash,
                chain_id,
            )?));
    ...
    fn handle_fill(log: &Log, transaction_hash: String, chain_id: u32) -> Result<SolverEvent> {
        ...
        let fill_event =
            OrderFillEvent::new(order_id, event.amountOutFilled, transaction_hash, chain_id);
