...
export function useBalance() {
  ...
  async function fetchEvmBalance(
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch EVM balance'
  ...
  async function fetchSolanaBalance(
    ...
    try {
      ...
      return {
        raw: balanceStr,
        formatted: accountInfo.value.uiAmountString || formatBalance(balanceStr, decimals),
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch Solana balance'
  ...
  function formatBalance(raw: string, decimals: number): string {
    ...
    const displayDecimal = trimmedDecimal.slice(0, 6) // Max 6 decimal places
